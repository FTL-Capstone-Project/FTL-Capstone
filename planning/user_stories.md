...
- **Story we cut (and why)**: We cut *"As a security analyst, I want reports stored in a searchable threat_reports table with indexed domains, so that lookups are fast."* It was written from the developer's perspective and prescribed a specific schema and index, which belongs in the database schema spec, not in a user story. The underlying user need is already covered by the plain-language search value in story #12.

- **Story that changed after Claude's feedback**: Analyst story #10 originally ended *"...and get back a chart."* Claude flagged "chart" as an implementation and output-format decision. We revised it to *"...and see the answer visualized,"* which keeps the real user benefit (understanding the threat landscape without SQL or expensive tooling) while leaving the exact rendering to the API contract and component spec.
