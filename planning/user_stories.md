...
**Maria, the cautious member with a security team**
Maria is a 52-year-old HR coordinator at a mid-sized company in Cleveland, Ohio. She lives in her inbox, receiving résumés, invoices, and benefits notices from people she's never met, so she can't just ignore unfamiliar senders. She's careful but not confident with technology. Her motivation is peace of mind. She's terrified of being the person who clicks the link that takes the company down, and she values knowing a real expert has her back. Her pain point is that when she reports something to IT she never hears back, so she never learns whether it was actually a malicious attack or not.

**Deshawn, the fast member with no security team**
...
- **Story we debated the scope of**: Organization member story #4 ("check a link quickly without derailing what I'm working on"). We originally wrote it as *"without leaving the email or window I'm working in,"* which quietly assumed a specific mechanism such as a browser extension or in-page overlay.
...
- **Redundancy we examined but kept**: Claude flagged individual story #1 ("check a suspicious link myself and get a clear verdict") and organization member story #5 ("an instant expert-level verdict on my own") as potentially redundant, since both are "get a verdict with no team behind me."
...
### Decisions from the Pod Sync (July 1, 2026)

After presenting to our mentors Sasha Titiyevskiy and Somal Vora, we recorded the following decisions.

- **Role rename: Employee to Organization Member**: Sasha suggested renaming the "Employee" role, floating "consultant" or "contractor." We discussed it as a pod and chose "Organization Member" instead. Both of our personas in this role (Maria, an HR coordinator, and Deshawn, a sales rep) are regular staff rather than external contractors, so "contractor" or "consultant" would have narrowed the role inaccurately. "Organization Member" captures Sasha's underlying point (this person acts inside a company, not for themselves) while staying true to who the personas actually are.

- **Browser extension confirmed as a stretch feature**: Somal flagged that offering three or four interaction methods (website, email forwarding, right-click extension) risked ballooning the scope. We committed to the website and email forwarding as the core interaction methods and moved the browser extension to a stretch feature.

- **MVP scope: one core problem per persona**: Somal advised us not to "boil the ocean," and Sasha reinforced defining a clear end-to-end MVP. We agreed to solve one primary problem per persona first (individual: is this link fake; organization member: fast in-context verdict; analyst: pre-scored reports on a dashboard) before adding secondary stories. We will finalize the exact MVP cut alongside the wireframes in Week 6.

- **Story we added after the Pod Sync**: We added analyst story #12 (organization-scoped visibility). Both mentors stressed that one organization must never see another organization's threat data, and we had no story covering it. We added it as a Security Analyst story because the analyst is the role that reads across an organization's full threat history, so the isolation boundary matters most there. This becomes a load-bearing constraint for the Week 6 data model.

- **Mentor-recommended build order (for the Week 6 plan)**: Somal recommended prioritizing the core link/email verification verdict first, then the analyst dashboard, then the campaign-clustering algorithm for duplicate URL "flavors." We will carry this ordering into the sprint plan. Sasha's overarching note was to agree on the architecture and task ownership before writing code, which is our explicit goal for the Week 6 detailed project plan.

## Wireframe

*To be added next week: low-fidelity mockups of the submission page, the individual and organization-member verdict report, and the analyst triage dashboard.*
